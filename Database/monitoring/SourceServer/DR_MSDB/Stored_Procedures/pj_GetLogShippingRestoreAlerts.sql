...
CREATE  PROCEDURE pj_GetLogShippingRestoreAlerts              
AS              
BEGIN              
...
WITH SecondaryInfo AS (  
  ...
  d.state ,  
        CASE   
            WHEN DATEDIFF(MINUTE, last_restored_date, GETDATE()) > (restore_threshold) and  d.state <>2 and is_in_standby=1 and d.state <>6 THEN 'Alert'  
...
WHERE d.name = s.secondary_database     )  
 ...
 INSERT INTO t_LogShippingAlerts  
...
-- Fetch only 'Secondary - Restore' alerts for online databases  
  SELECT               
        ...
        s.RestoreAlertStatus AS AlertStatus,              
        s.ErrorMessage AS ErrorMessage,              
        s.AlertDate -- Include the alert date              
    FROM               
        SecondaryInfo s   
WHERE   
    s.RestoreAlertStatus in ('Alert','Recovery','Restoring')  
ORDER BY   
    DatabaseName, Source;  
END; 
