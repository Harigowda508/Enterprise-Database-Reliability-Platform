Create view v_InstantBackupJobs as  
  ...
  --sysjobs.enabled AS job_enabled,  
        ...
        -- Error message if job failed  
         (SELECT TOP 1 step_history.message       FROM msdb.dbo.sysjobhistory step_history  
         WHERE step_history.job_id = sysjobs.job_id           
         AND step_history.step_id = 1 -- Get only the 1st step error            AND step_history.run_status = 0   
    ...
    WHERE   
        -- Filter for relevant job names and exclude Log Shipping  
        (  
            sysjobs.name LIKE '%backup%'   
