...
# Select the latest CSV file for archival
$ReportFile = Get-ChildItem -Path "D:\DATAEXT\OUTPUT\Daily_Archival_Report_Failed.csv" |
...
if ($ReportFile) {
    ...
    if ($ReportContent.Count -ne 0) {
        ...
        $TeamsMessage = "**SQL Archival Failure Alert**`n`n"
        $TeamsMessage += "| Job Name | Enabled | Frequency | Status | Failed Date | Failed Time | Error Message |`n"
        $TeamsMessage += "|----------|---------|-----------|--------|-------------|--------------|----------------|`n"

        foreach ($row in $ReportContent) {
            $TeamsMessage += "| $($row.job_name) | $($row.job_enabled) | $($row.frequency) | $($row.job_status) | $($row.succeeded_date) | $($row.succeeded_time) | $($row.error_message) |`n"
        }
        ...
        $BodyJson = ConvertTo-Json -Depth 4 -Compress -InputObject @{
            ...
            summary   = "SQL Archival Job Failure Notification"
            themeColor = "FF0000"
            title     = "❗ SQL Archival Failure Notification"
            text      = $TeamsMessage
        }
        ...
        Write-Host "Teams archival alert sent successfully!"
    } else {
        Write-Host "No valid archival failure data found. Alert not sent."
    }
} else {
    Write-Host "No archival failure CSV report file found. Alert not sent."
}
