...
$ReportFile = Get-ChildItem -Path "D:\DATAEXT\OUTPUT\Daily_Backup_Report_Failed.csv" |
...
if ($ReportFile) {
    ...
    if ($ReportContent.Count -ne 0) {
        ...
        $TeamsMessage = "**SQL Backup Failure Alert**`n`n"
        $TeamsMessage += "| Job Name | Status | Failed Date | Failed Time | Error Message |`n"
        $TeamsMessage += "|----------|--------|-------------|--------------|----------------|`n"

        foreach ($row in $ReportContent) {
            $TeamsMessage += "| $($row.job_name) | $($row.job_status) | $($row.failed_date) | $($row.failed_time) | $($row.error_message) |`n"
        }
        ...
        $BodyJson = ConvertTo-Json -Depth 4 -Compress -InputObject @{
            ...
            summary = "SQL Job Failure Notification"
            themeColor = "FF0000"
            title = " SQL Backup Failure Notification"
            text = $TeamsMessage
        }
    ...
    Write-Host "No failed backup data found. Alert not sent."
}
